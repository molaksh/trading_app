"""
Crypto exchange adapter - Phase 0 stub.
...
class CryptoAdapter(BrokerAdapter):
    """Crypto exchange adapter (stub for Phase 0)."""
    
    def __init__(self, paper_mode: bool = True):
        """Initialize crypto adapter."""
        self.paper_mode = paper_mode
        self.mode = "paper" if paper_mode else "live"
        logger.info(f"CryptoAdapter initialized (mode={self.mode})")
    ...
    def account_equity(self) -> float:
        ...
        raise NotImplementedError("Crypto adapter not implemented in Phase 0")
