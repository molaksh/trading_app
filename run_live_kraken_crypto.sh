...
# Map ChatGPT key to OpenAI key if needed
if [ -z "$OPENAI_API_KEY" ] && [ -n "$CHATGPT_API_KEY" ]; then
...
AI_MAX_CALLS_PER_DAY="${AI_MAX_CALLS_PER_DAY:-3}"
AI_VALIDATE_SCHEDULER="${AI_VALIDATE_SCHEDULER:-false}"
...
docker run -d \
  ...
  -e KRAKEN_API_SECRET="$KRAKEN_API_SECRET" \
    ...
    -e AI_ADVISOR_ENABLED="$AI_ADVISOR_ENABLED" \
    -e AI_MAX_CALLS_PER_DAY="$AI_MAX_CALLS_PER_DAY" \
    -e AI_VALIDATE_SCHEDULER="$AI_VALIDATE_SCHEDULER" \
  ...
  -e PHASE_G_DRY_RUN="${PHASE_G_DRY_RUN:-true}" \
  -e PHASE_H_CRYPTO_ENABLED="${PHASE_H_CRYPTO_ENABLED:-true}" \
  -e PHASE_H_CRYPTO_LIVE_AUTONOMY="${PHASE_H_CRYPTO_LIVE_AUTONOMY:-false}" \
