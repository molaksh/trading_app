...
status=$(docker inspect -f '{{.State.Status}}' paper-nse-swing-india 2>/dev/null || true)
restarting=$(docker inspect -f '{{.State.Restarting}}' paper-nse-swing-india 2>/dev/null || true)
restart_count=$(docker inspect -f '{{.RestartCount}}' paper-nse-swing-india 2>/dev/null || echo 0)
...
logs=""
while true; do
  logs=$(docker logs paper-nse-swing-india 2>&1 || true)
  if [ -n "$logs" ]; then
...
if echo "$logs" | grep -Eiq "Market data provider initialized: ($ALLOWED_PROVIDERS_REGEX)|NSEProvider initialized"; then
